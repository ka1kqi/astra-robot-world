...
Free-text requests support toppling, object displacement, gripper circles, extraction from underneath another object, and object rotation. Extraction names the lower object, upper object, and landing surface: the lower must move and separate horizontally by at least 4 cm by default, the upper must lose its lower support and land on the named surface, and both must settle for 0.5 seconds. Unrelated objects, including the landing surface, remain preserved. Optional destination coordinates further constrain the lower object's final position. It does not yet verify arbitrary gestures, exact container containment, custom force limits, or general ordered compound goals.

`pick_place` accepts optional `target_rotation`: an absolute object orientation in intrinsic XYZ radians. It currently supports turns around world Z that preserve the current flat support face; tilting is rejected before motion. The robot physically lifts, turns, and replaces the object, then checks orientation within 5 degrees and stable placement for 0.5 seconds. A `rotate` Action Lab goal adds a full 3D angular-error evaluator (5 degrees by default), preserves the initial object center within the goal’s position tolerance unless a destination is supplied, and requires the same three verified trials before saving.
